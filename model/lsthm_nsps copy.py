...
class CrossAttention2(nn.Module):
    def __init__(self, dh1, dh2, dk, dv, attn_dropout=0.2):
        ...
        self.Wq = nn.Parameter(torch.ones(self.dh1, self.dk))  # D1 * Dk
        self.Wk = nn.Parameter(torch.ones(self.dh2, self.dk))  # D2 * Dk
        self.Wv = nn.Parameter(torch.ones(self.dh2, self.dv))  # D2 * Dv
        
        self.dropout = nn.Dropout(attn_dropout)

        self.layer_norm = nn.LayerNorm(self.dv, eps=1e-6)
...
class MARN1_nsps(nn.Module):
    def __init__(self, n_classes, dataset):
        ...
        final_out = 2 * (self.total_h_dim) + self.d_l + self.d_a
        ...
        # cross attention
        self.crossatt_l2a = CrossAttention2(self.d_l, self.d_a, self.d_l, self.d_l)
        self.crossatt_a2l = CrossAttention2(self.d_a, self.d_l, self.d_a, self.d_a)
