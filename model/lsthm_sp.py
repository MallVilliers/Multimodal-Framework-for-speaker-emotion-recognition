...
class LSTHM1(nn.Module):
    ...
    def forward(self, x, ctm, htm, ztm):
        ...
        sums = input_affine + output_affine + hybrid_affine # N, 4*dim
...
class MARN_cell(nn.Module):
    def __init__(self, dh_l, dh_a, d_l, d_a, dropout=0.5) -> None:
        ...
        self.lsthm_q = LSTHM1(self.dh_a, self.dh_s, self.dh_l, self.dh_s)
    ...
    def forward(self, x, x_l, x_a, qmask):
        ...
        for i in range(T):
            ...
            q0_sel = self._select_parties(q, qm_idx)  # B, D
            # 更新当前说话人状态
            c_q, h_q = self.lsthm_q(q0_sel, *(c_q, h_q, z_l))  # B, D
            h_q = self.dropout(h_q)
            # 更新q
            q_s=h_q.unsqueeze(1).expand(-1, qmask[i].size()[1],-1)
            q_0= q0_sel.unsqueeze(1).expand(-1, qmask[i].size()[1],-1)  # N, 2, D
            qmask_ = qmask[i].unsqueeze(2) #qmask -> batch, party      N, 2, 1
            q = q_0 * (1 - qmask_) + q_s * qmask_      # N, 2, D

            # current time step
            c_l, h_l = self.lsthm_l(x_l[i], *(c_l, h_l, z_l))  # B, D
            h_l = self.dropout(h_l)
            c_a, h_a = self.lsthm_a(x_a[i], *(c_a, h_a, z_l))  # B, D
    ...
    def cal_q(self, qtm, qmask, h_s, c_s):  # TODO: 说话人也加上模态信息，三个lsthm
...
class MARN1_sp(nn.Module):
    def __init__(self, n_classes):
        super(MARN1_sp, self).__init__()
        ...
        final_out = 2 * (self.total_h_dim + self.dh_l+ self.dh_l) + self.dh_l + self.dh_l
        ...
        self.crossatt_l2a = CrossAttention2()
        self.crossatt_a2l = CrossAttention2()
