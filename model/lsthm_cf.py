...
class MARN_cell(nn.Module):
    ...
    def forward(self, x, x_l, x_a, qmask):
        ...
        for i in range(T):
            q, q_affine, h_s, c_s = self.cal_q(q, qmask[i], h_s, c_s)    # [B, 2, dh], [B, 4*dh]
            # current time step
            c_l, h_l = self.lsthm_l(x_l[i], *(c_l, h_l, z_l, q_affine))  # B, D
            c_a, h_a = self.lsthm_a(x_a[i], *(c_a, h_a, z_l, q_affine))  # B, D
            z_l = self.crossatt_l2a(c_l, c_a)  # B, D
            # # z_a = self.crossatt_a2l(c_a, c_l)   # B, D

            all_hs=torch.cat([h_l, h_a, z_l, q_affine], dim=1)
...
class MARN1_cf(nn.Module):
    def __init__(self, n_classes):
        super(MARN1_cf, self).__init__()
        ...
        final_out = 2 * (self.total_h_dim + self.dh_l+ self.dh_l) + 128
    ...
    def forward(self, x, qmask, umask):
        ...
        output = F.log_softmax(self.nn_out(torch.cat([h, attn1], dim=-1)), 2)
