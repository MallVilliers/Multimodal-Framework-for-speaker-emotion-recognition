...
class MatchingAttention(nn.Module):
    ...
    def forward(self, M, x, mask=None):
        ...
        if self.att_type=='dot':
            ...
            alpha = F.softmax(torch.bmm(x_, M_), dim=2) # batch, 1, seqlen
        elif self.att_type=='general2':  #加了mask并且归一化的general
            M_ = M.permute(1,2,0) # batch, mem_dim, seqlen
            x_ = self.transform(x).unsqueeze(1) # batch, 1, mem_dim
...
class DialogueRNNCell(nn.Module):
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        ...
        if self.listener_state:
            ...
            ss_ = self._select_parties(qs_, qm_idx).unsqueeze(1).expand(-1,qmask.size()[1],-1).contiguous().view(-1,self.D_p)
...
class CNNFeatureExtractor(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, output_size, filters, kernel_sizes, dropout):
        ...
        self.convs = nn.ModuleList([nn.Conv1d(in_channels=embedding_dim, out_channels=filters, kernel_size=K) for K in kernel_sizes]) #用size=3,4,5的卷积核分别卷
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x, umask):
        ...
        num_utt, batch, num_words = x.size()  #句子数，batch,句子中的词数
        
        x = x.type(LongTensor)  # (num_utt, batch, num_words)  #转tensor
        ...
        convoluted = [F.relu(conv(emb)) for conv in self.convs] #（num_att*batch,50,xx)*3
        pooled = [F.max_pool1d(c, c.size(2)).squeeze() for c in convoluted] #(num_att*batch,50,1)*3
