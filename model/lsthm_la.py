...
class MARN_cell(nn.Module):
    def __init__(self, dh_l, dh_a, d_l, d_a) -> None:
        ...
        self.lstm_la_l = nn.LSTMCell(self.d_l, self.d_l)
        self.lstm_la_a = nn.LSTMCell(self.d_a, self.d_a)
    ...
    def forward(self, x, qmask):
        ...
        h_la_l = torch.zeros(N, self.d_l).to(x.device)
        c_la_l = torch.zeros(N, self.d_l).to(x.device)
        h_la_a = torch.zeros(N, self.d_a).to(x.device)
        c_la_a = torch.zeros(N, self.d_a).to(x.device)
        ...
        for i in range(T):
            ...
            # current time step
            h_la_l, c_la_l = self.lstm_la_l(x_l[i], (h_la_l, c_la_l))   # B, D
            h_la_a, c_la_a = self.lstm_la_a(x_l[i], (h_la_a, c_la_a))

            c_l, h_l = self.lsthm_l(F.softmax(c_la_l, dim=-1) * x_l[i], *(c_l, h_l, x_la[i], q_affine))  # B, D
            c_a, h_a = self.lsthm_a(F.softmax(c_la_a, dim=-1) * x_a[i], *(c_a, h_a, x_al[i], q_affine))  # B, D
...
class MARN1_la(nn.Module):
    def __init__(self, n_classes):
        super(MARN1_la, self).__init__()
